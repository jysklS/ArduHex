...
void AP_MotorsMatrix::output_armed_stabilizing()
{
    ...
    float   forward_thrust;             // JL
    float   lateral_thrust;             // JL
    ...
    forward_thrust = _forward_in * get_compensation_gain();     //JL
    lateral_thrust = _lateral_in * get_compensation_gain();     //JL
    ...
    for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            ...
            _fwlat_out[i] = forward_thrust * _forward_factor[i] + lateral_thrust * _lateral_factor[i];              //JL
    ...
    for (i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            _thrust_rpyt_out[i] = throttle_thrust_best_rpy + thr_adj + rpy_scale*_thrust_rpyt_out[i] + _fwlat_out[i]; //JL
        }
    }
...
//JL
void AP_MotorsMatrix::add_motor_raw_6dof(int8_t motor_num, float roll_fac, float pitch_fac, float yaw_fac, float forward_fac, float lateral_fac, uint8_t testing_order)
{
    // ensure valid motor number is provided
    if( motor_num >= 0 && motor_num < AP_MOTORS_MAX_NUM_MOTORS ) {
        ...
        case MOTOR_FRAME_HEXA:
            switch (frame_type) {
                ...
                case MOTOR_FRAME_TYPE_X:
                    ...
                    success = true;
                    break;
                case MOTOR_FRAME_TYPE_FULLY_ACT: //JL
                    add_motor_raw_6dof(AP_MOTORS_MOT_1, -1.0f,                0.0f,  1.0f, -1.0f,                0.0f, 2);
                    add_motor_raw_6dof(AP_MOTORS_MOT_2,  1.0f,                0.0f, -1.0f, -1.0f,                0.0f, 5);
                    add_motor_raw_6dof(AP_MOTORS_MOT_3,  0.5f,  0.866025403784439f,  1.0f,  0.5f,  0.866025403784439f, 6);
                    add_motor_raw_6dof(AP_MOTORS_MOT_4, -0.5f, -0.866025403784439f, -1.0f,  0.5f,  0.866025403784439f, 3);
                    add_motor_raw_6dof(AP_MOTORS_MOT_5, -0.5f,  0.866025403784439f, -1.0f,  0.5f, -0.866025403784439f, 1);
                    add_motor_raw_6dof(AP_MOTORS_MOT_6,  0.5f, -0.866025403784439f,  1.0f,  0.5f, -0.866025403784439f, 4);
                    success = true;
                    break;
                default:
